...
- take a string of digits and a parameter n
- output all the possible strings of consecutive digits that are 'n' digits long
- raise error if 'n' is larger than number of digits of input string

Logic

- assess length of input string
- iterate through digits of string, using Fixnum#upto
- should iterate up to (string length - n)
- use String#[] with range to get slices of string
...
class Series
  ...
  def slices(n)
    raise ArgumentError unless n.instance_of?(Fixnum)
    raise ArgumentError if n > @digit_string.size
    separated_digits(n)
  end

  def separated_digits(n)
    output_array = []
    iterator_array = 0.upto(@digit_string.size - n)
    iterator_array.each do |iterator| 
      digits_to_add = @digit_string[(iterator...(iterator + n))]
      output_array << digits_to_add.to_s.chars.map(&:to_i)
    end
    output_array
  end
